...
# Subtract 1 needed to get values indexing from 0 for our tensor
...
for train_index, val_index in k_folds.split(features_tensor,label_tensor):
    ...
    plt.title(f"(MLP) Fold {fold_n} - Train & Validation Loss")
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.legend()
    plt.grid(True)
    ...
    plt.title(f"(MLP) Fold {fold_n} - Train & Validation Accuracy")
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy (%)")
    plt.legend()
    plt.grid(True)
...
with open("MLP_report/mlp_report.txt", "w") as file:
    file.write("---------MLP Report---------\n")
